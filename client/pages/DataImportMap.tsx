...
import { AlertTriangle, Upload, X, Settings, ChevronDown, Send, FileText, CheckCircle, AlertCircle } from "lucide-react";
...
const availableCaptions = [
  "Reference", "Org Unit", "Forename(s)", "Surname", "Email", "Job Title", 
  "Manager Name", "Phone", "Department", "Location", "Start Date", "Employee ID",
  "First Name", "Last Name", "Full Name", "Username", "Role", "Status"
];
...
export default function DataImportMap() {
  ...
  const analyzeCSVData = useCallback((data: string[][]) => {
  ...
  const generateAIResponse = (userInput: string, currentMappings: MappingRow[], columns: CSVColumn[]): string => {
                        ...
                        <tr key={row.id} className="border-t border-gray-300">
                            ...
                            >
                              ...
                              <SelectContent>
                                {availableCaptions.map(caption => (
                                  <SelectItem key={caption} value={caption}>{caption}</SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
                          </td>
                          <td className="p-2 border-r border-gray-300 text-center">
                            <Checkbox 
                              ...
                              onCheckedChange={(checked) => updateMappingRow(row.id, 'keyField', checked)}
                              className="w-5 h-5"
                            />
                          </td>
                          <td className="p-2 border-r border-gray-300 text-center">
                            <Checkbox 
                              ...
                              onCheckedChange={(checked) => updateMappingRow(row.id, 'matchById', checked)}
                              className="w-5 h-5"
                            />
                          </td>
